...
$(document).on('click', '.product-wrap .favorite-btn', function() {
  ...
  const favoriteData = {
    img: wrap.children('.product-img').attr('src'),
  ...
  if(subject === 'removeFavorite') {
    $(`.all-products .product-wrap .product-name[href="${favoriteData.link}"]`).parent().siblings('.favorite-btn').removeClass('active')
  } else if (subject === 'setFavorite') {
    $(`.all-products .product-wrap .product-name[href="${favoriteData.link}"]`).parent().siblings('.favorite-btn').addClass('active')
  }
  chrome.tabs.query({
    active: true,
    currentWindow: true
